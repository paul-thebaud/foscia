...
Check if the given keys are different between two snapshots.
If no keys are provided, will check whole snapshots (including IDs).
...
- [`{ModelSnapshot<I>}`](/docs/reference/api/modules/core#modelsnapshot) `nextSnapshot`
- [`{ModelSnapshot<I>}`](/docs/reference/api/modules/core#modelsnapshot) `prevSnapshot`
- [`{ArrayableVariadic<ModelKey<I>>}`](/docs/reference/api/modules/core#modelkey) `...only`

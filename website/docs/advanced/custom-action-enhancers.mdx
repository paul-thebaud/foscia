...
from any existing FuncClient enhancers.

## Goal

Since FuncClient is pagination agnostic, providing a `first` enhancer is
...
[**custom runners guide**](/docs/advanced/custom-action-runners).
...
import { Action, Model, ModelInstance, forModel } from 'func-client/core';
...
Once your enhancer is ready, you may use it like any other FuncClient enhancer.
...
[as any other enhancers of FuncClient](/docs/actions#extensions).

```ts title="action/enhancers/first.ts"
import {Action, ActionParsedExtension, ConsumeId, ConsumeModel, ConsumeType, forModel, makeEnhancersExtension, Model, ModelInstance} from 'func-client/core';
import {paginate} from 'func-client/jsonapi';
...
type FirstEnhancerExtension = ActionParsedExtension<{
  first<Context extends {}, Extension extends {}, ContextModel extends Model>(
    ...
    model: ContextModel,
  ): Action<Context & ConsumeModel<ContextModel> & ConsumeType & ConsumeId, Extension>;
}>;
...
Check out [the custom runner guide](/docs/advanced/custom-action-runners) which

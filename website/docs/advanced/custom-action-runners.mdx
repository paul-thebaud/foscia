...
Since Foscia is pagination agnostic, providing a `first` runner is
...
import {Action, ConsumeAdapter, ConsumeDeserializer, ConsumeModel, Model, one} from 'foscia/core';
...
[as any other runners of Foscia](/docs/actions#extensions).

```ts title="action/runners/first.ts"
import {Action, ActionParsedExtension, ConsumeAdapter, ConsumeDeserializer, ConsumeModel, Model, makeRunnersExtension, one} from 'foscia/core';
import {paginate} from 'foscia/jsonapi';

...
| `sync`        | `boolean`&vert;`'retrieve'`&vert;`'write'` | `true`      | Avoid retrieving/writing the property's value from/to the backend.           |
...
[**advanced guide on transformers**](/docs/guides/models/custom-transformers) to

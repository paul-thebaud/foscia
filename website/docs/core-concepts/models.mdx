...
-   When using TypeScript, it allows only importing type of the class using
    `import type Post from './models/post'`, and thus avoiding circular
    dependencies when models have circular relationships
-   It gives more flexibility as you can quickly add custom properties or
    methods in the future

That being said, you can use the syntax you prefer.
...
through actions, so you may [read the actions guide](/docs/core-concepts/actions)
...
| `readOnly`     | `boolean`      | `false`     | Makes the property read-only.                                                         |

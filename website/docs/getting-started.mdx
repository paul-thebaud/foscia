...
[JSON:API implementation page](/docs/implementations/jsonapi).
...
You can read more details on the
[JSON REST implementation page](/docs/implementations/jsonrest).
...
instead of `action().use(model(Post))`), you should provide the extensions
...
```js
import { makeJsonApi, coreExtensions, crudExtensions, jsonApiExtensions } from 'func-client/blueprints';
...
```js
import { makeJsonRest, coreExtensions, crudExtensions, jsonRestExtensions } from 'func-client/blueprints';

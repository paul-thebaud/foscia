...
```javascript
import { forModel, when } from 'func-client/core';
import {
    filterBy,
    sortByDesc,
    paginate,
...
```javascript
import { forModel, instance, include, when, oneOrFail } from 'func-client/core';
...
the given path if it is not an "absolute" (starting with `/`) path. This allows
you to run non-standard actions scoped to an instance, etc.
